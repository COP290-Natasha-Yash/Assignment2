...
router.post('/:id/boards/:boardId/columns/:columnId/tasks', requireProjectRole(['ADMIN', 'MEMBER']),async (req: Request, res: Response) => {
    ...
    if (!title){
        res.status(400).json({error: {message: 'Title is Required', code: 'BAD_REQUEST'}});
        return;
    }

    if (!reporterId){
        res.status(400).json({error:{message: 'Reporter ID is required', code: 'BAD_REQUEST'}})
        return;
    }
    ...
    const task = await prisma.task.create({data: {title, description, priority, type, assigneeId, reporterId, dueDate, columnId, parentId }})
