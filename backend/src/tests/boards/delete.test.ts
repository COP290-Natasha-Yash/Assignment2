...
describe('DELETE /api/projects/:id/boards/:boardId', () => {
  it('1. Should fail if not project admin', async () => {
  ...
  it('2. Should fail if board not found', async () => {
    const response = await request(app)
      .delete(`/api/projects/${projectId}/boards/invalidid`)
  ...
  it('3. Should fail if board belongs to different project', async () => {
    ...
    const response = await request(app)
      .delete(`/api/projects/${projectId}/boards/${otherBoard.id}`)
      .set('Cookie', adminCookie);
    expect(response.status).toBe(404);
    expect(response.body.error.code).toBe('NOT_FOUND');
  });

  it('4. Should delete board successfully', async () => {
    const response = await request(app)
      .delete(`/api/projects/${projectId}/boards/${boardId}`)
      .set('Cookie', adminCookie);
